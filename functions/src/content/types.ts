 ...
 *
 * This is a faithful mirror of `src/content/types.ts` (the client content model).
 * It lives inside `functions/src/` because the functions `tsconfig.json` sets
 * `rootDir: "src"`, so we cannot import the client copy directly. Keeping it
 * byte-for-byte aligned means the server's verification firewall (`engine.ts`)
 * and generation output match exactly what the client renders/grades.
 *
 * Only addition vs. the client copy: `ProblemStep.source` (PRD §3.4 — additive,
 * backward-compatible). The client team adds the same field in their slice.
 */
